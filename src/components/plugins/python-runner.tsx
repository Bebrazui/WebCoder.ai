...
export function PythonRunner() {
  const [inputValue, setInputValue] = useState('{"value": 10}');
  const [entryPoint, setEntryPoint] = useState('my_script.py');
  ...
  return (
    <div className="flex flex-col h-full bg-background text-foreground">
      <div className="p-2 border-b border-border">
        <h2 className="text-lg font-headline font-semibold">Python Script Runner</h2>
      </div>
      ...
      <ScrollArea className="flex-grow">
        <div className="p-4 space-y-4">
            <Alert>
                ...
                <AlertDescription>
                    <p>This tool sends your current project files to a secure server environment to run the Python script. Your files are not stored on the server after execution.</p>
                </AlertDescription>
            </Alert>
          <div className="space-y-2">
            ...
            <Input
              id="entry-point"
              placeholder="e.g., main.py or src/app.py"
              value={entryPoint}
          ...
          {result && (
            <div className="space-y-2 pt-4">
              <Label>Result from Python</Label>
